...
public class MethodReferenceTypes {
	...
	public static void boundMethodReferences() {
		...
		// No need to say which instance to call it on - the supplier is bound to 'name' from above definition
		...
		// No need to say which instance to call it on - the predicate is bound to 'name' from above definition
		System.out.println(titleL.test("Mr.")); 	// true
		System.out.println(titleMR.test("Ms.")); 	// false
	}
	
	private static void unboundMethodReferences() {
		/* Function<T, R>
		 * 		R apply(T)
		 * 			String apply(String)
		 */
		...
		System.out.println(upperL.apply("Robbie"));		// ROBBIE
		System.out.println(upperMR.apply("Robbie"));	// ROBBIE
		...
		// Since the above input & return params are all of teh same type, the BinaryOperator could be used here, instead of BiFunction
		BinaryOperator<String> concatBOL = (s1, s2) -> s1.concat(s2);	// lambda
		BinaryOperator<String> concatBOMR = String::concat;				// method reference
		
		System.out.println(concatL.apply("Robbie ", "Higgins"));		// Robbie Higgins
		
		// 1st parameter is used for execusting the instance method
		// "Robbie".concat("Higgins")		
		System.out.println(concatMR.apply("Robbie ", "Higgins"));		// Robbie Higgins		
	}
	...
	private static void staticMethodReferences() {
		...
		sortL.accept(listOfNumbers);	// execution (lambda)
		System.out.println(listOfNumbers); 		// [1, 2, 4, 5, 9]
		
		listOfNumbers = Arrays.asList(8, 12, 4, 3, 7);
		sortMR.accept(listOfNumbers);	// execution (method reference)
